...
$this->title = 'Таннарх буйича хисобот' . Yii::$app->formatter->asDatetime($start, "php:d.m.Y") . ' - ' . Yii::$app->formatter->asDatetime($end, "php:d.m.Y");
...
<div class="report-index">
    ...
    <div class="mt-2">
        <table class="table table-bordered text-center table-sm" id="tbl_exporttable_to_xls_1">
            ...
            $total4 = 0;
            ?>
            <?php $i=1; foreach ($credits as $n=>$item):?>
                <tr>
                    <td><?php echo $i; ?></td>
...
<script>
    ...
    function setPsumma() {
        ...
        document.querySelectorAll('.ost').forEach(element => {
            ...
            total_ost += +(element.textContent.replaceAll(' ', ''))
        });
        document.querySelector('#totalOst').textContent = numberWithCommas(total_ost);
    }
    setPsumma();
